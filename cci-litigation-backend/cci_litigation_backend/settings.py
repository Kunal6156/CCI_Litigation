...
REST_FRAMEWORK = {
    ...
    'PAGE_SIZE': 20, # Increased page size for better UX
    ...
    'DEFAULT_THROTTLE_RATES': {
        'anon': '100/hour',
        'user': '1000/hour',
        'login': '10/min',  # Custom rate for login
    }
}

# Simple JWT settings
SIMPLE_JWT = {
    'ACCESS_TOKEN_LIFETIME': timedelta(minutes=120), # Extended to 2 hours for better UX
    'REFRESH_TOKEN_LIFETIME': timedelta(days=7),   # Refresh token valid for 7 days
    'ROTATE_REFRESH_TOKENS': True, # New refresh token with each refresh request
    'BLACKLIST_AFTER_ROTATION': True,
    'UPDATE_LAST_LOGIN': True,
    'ALGORITHM': 'HS256',
    'SIGNING_KEY': SECRET_KEY, # This already uses SECRET_KEY
    'VERIFYING_KEY': None,
    'AUDIENCE': None,
    'ISSUER': None,
    'JWK_URL': None,
    ...
    'TOKEN_OBTAIN_SERIALIZER': 'litigation_api.serializers.MyTokenObtainPairSerializer', # Custom serializer
}
...
DEFAULT_FROM_EMAIL = os.environ.get('DEFAULT_FROM_EMAIL', 'noreply@ccilitigation.com')

# Custom application settings for CCI Litigation Platform
CCI_LITIGATION_SETTINGS = {
    # Department user limits
    'DEPARTMENT_USER_LIMITS': {
        'corporate_office': 4,  # 2 admin + 2 regular
        'tandur': 2,
        'rajban': 2,
        'bokajan': 2,
        'akaltara': 1,
    ...
    # Admin limits per department
    'ADMIN_USER_LIMITS': {
        'corporate_office': 2,  # Max 2 admins only in Corporate Office
    },
...
# Session settings
SESSION_COOKIE_AGE = CCI_LITIGATION_SETTINGS['SESSION_TIMEOUT_MINUTES'] * 60  # Convert to seconds
